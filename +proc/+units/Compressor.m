classdef Compressor < handle
    properties
        ...
        PR = NaN
        eta_isentropic = 0.75
        power_kW = NaN              % positive = power input
    end

    methods
        function obj = Compressor(inlet, outlet, varargin)
        ...
        function eqs = equations(obj)
            ...
            h2target = h1 + (h2s - h1)/max(obj.eta_isentropic, 1e-8);

            h2 = obj.outlet.h(obj.outlet.T, 'sensible');
            switch lower(char(obj.mode))
                case 'power'
                    eqs(end+1) = obj.power_kW - obj.outlet.n_dot*(h2 - h1)/3600;
                otherwise
        ...
        function str = describe(obj)
            str = sprintf('Compressor: %s -> %s (%s)', string(obj.inlet.name), string(obj.outlet.name), obj.mode);
        end
        ...
        function Pout = resolvePout(obj)
            switch lower(char(obj.mode))
                case {'pouteta','power'}
                    if ~isfinite(obj.Pout), error('Compressor requires Pout in mode %s.', obj.mode); end
                    Pout = obj.Pout;
                case 'preta'
                    if ~isfinite(obj.PR), error('Compressor PREta mode requires PR.'); end
                    Pout = obj.inlet.P * obj.PR;
                otherwise
                    error('Unknown compressor mode %s', obj.mode);
            end
        end
