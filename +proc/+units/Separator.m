classdef Separator < handle
    properties
        ...
        outletB    % Stream object (e.g. liquid / water product)
    ...
    methods
        function obj = Separator(inlet, outletA, outletB, phi)
            % inlet: Stream
            % outletA/outletB: Stream
            % phi: vector, phi(i) = fraction of species i going to outletA
            obj.inlet   = inlet;
            obj.outletA = outletA;
            obj.outletB = outletB;
            obj.phi     = phi(:).'; % row vector
        end
        ...
        function eqs = equations(obj)
            ...
            % For each species i:
            % outletA.n_dot * outletA.y(i) = phi(i) * inlet.n_dot * inlet.y(i)
            % outletB.n_dot * outletB.y(i) = (1-phi(i)) * inlet.n_dot * inlet.y(i)
            for i = 1:ns
            ...
            % --- Mole fraction sum constraints ---
            eqs(end+1) = sum(obj.outletA.y) - 1;
            eqs(end+1) = sum(obj.outletB.y) - 1;

            % --- Simple T/P behaviour (no change) ---
